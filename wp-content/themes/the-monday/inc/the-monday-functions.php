...
function the_monday_scripts_styles() {
    ...
    wp_enqueue_script( 'lightSlider-script', THE_MONDAY_JS_URL . '/lightslider.js', array(), '1.1.5' );

	wp_enqueue_script( 'the-monday-imagejs', THE_MONDAY_JS_URL . '/imagesloaded.pkgd.js', array(), '3.1.8' );
    
    wp_enqueue_script( 'the-monday-smoothscroll', THE_MONDAY_JS_URL . '/smoothscroll.js', array(), '1.3.8' );
...
function the_monday_body_class( $classes ) {
    ...
    $the_monday_header_type = get_theme_mod( 'the_monday_header_type', 'image' );
    if( $the_monday_header_type == 'image' && is_front_page() ) {
        $classes[] = 'home-header-image';
    }
